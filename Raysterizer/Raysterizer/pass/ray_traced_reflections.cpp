...
namespace Raysterizer
{
    namespace Pass
    {
        ...
        struct ReflectionRayTracePushConstants
        {
            float    bias;
            float    trim;
            uint32_t num_frames;
        ...
        struct ReflectionTemporalAccumulationPushConstants
        {
        ...
        struct ReflectionATrousFilterPushConstants
        {
            int     radius;
            int     step_size;
            float   phi_color;
        ...
        struct ReflectionUpsamplePushConstants
        {
            int32_t g_buffer_mip;
        };
        ...
        Error RayTracedReflections::Setup(std::shared_ptr<CommonResources> common_resources_)
        {
            ...
            scale = common_resources->reflections.scale;
            ...
            temporal_accumulation.current_output_image.clear();
            temporal_accumulation.current_output_view.clear();
            temporal_accumulation.current_output_texture.clear();
            ...
            // Create images
            {
                // Ray Trace
                {
                    ray_trace.image = CommonResources::CreateImage(vk::ImageType::e2D, width, height, 1, 1, 1,
                    ...
                    c.SetName(ray_trace.image, fmt::format("Reflections Ray Trace Image"));

                    ray_trace.view = CommonResources::CreateImageView(ray_trace.image, vk::ImageViewType::e2D, vk::ImageAspectFlagBits::eColor);
                    c.SetName(ray_trace.view, fmt::format("Reflections Ray Trace Image View"));
                ...
                // Reprojection
                {
                    ...
                    {
                        auto current_output_image = CommonResources::CreateImage(vk::ImageType::e2D, width, height, 1, 1, 1,
                            vk::Format::eR16G16B16A16Sfloat, VMA_MEMORY_USAGE_GPU_ONLY,
                        ...
                        c.SetName(current_output_image, fmt::format("Reflections Reprojection Moments Image {}", i));

                        auto current_output_view = CommonResources::CreateImageView(current_output_image, vk::ImageViewType::e2D, vk::ImageAspectFlagBits::eColor);
                        c.SetName(current_output_view, fmt::format("Reflections Reprojection Moments Image View {}", i));

                        auto current_output_texture = std::make_shared<Texture>(current_output_image, current_output_view, nearest_sampler);

                        temporal_accumulation.current_output_image.emplace_back(current_output_image);
                        temporal_accumulation.current_output_view.emplace_back(current_output_view);
                        temporal_accumulation.current_output_texture.emplace_back(current_output_texture);

                        auto current_moments_image = CommonResources::CreateImage(vk::ImageType::e2D, width, height, 1, 1, 1,
                            vk::Format::eR16G16B16A16Sfloat, VMA_MEMORY_USAGE_GPU_ONLY,
                            vk::ImageUsageFlagBits::eTransferDst | vk::ImageUsageFlagBits::eTransferSrc | vk::ImageUsageFlagBits::eStorage | vk::ImageUsageFlagBits::eSampled, vk::SampleCountFlagBits::e1);
                        c.SetName(current_moments_image, fmt::format("Reflections Reprojection Moments Image {}", i));
                        ...
                        c.SetName(current_moments_view, fmt::format("Reflections Reprojection Moments Image View {}", i));
                    ...
                    c.SetName(temporal_accumulation.prev_image, fmt::format("Reflections Previous Reprojection Image"));
                    ...
                    c.SetName(temporal_accumulation.prev_view, fmt::format("Reflections Previous Reprojection Image View"));
                ...
                // Upsample
                {
                    ...
                    upsample.image = CommonResources::CreateImage(vk::ImageType::e2D, swapchain_width, swapchain_height, 1, 1, 1,
                        vk::Format::eR16G16B16A16Sfloat, VMA_MEMORY_USAGE_GPU_ONLY,
                        vk::ImageUsageFlagBits::eStorage | vk::ImageUsageFlagBits::eSampled, vk::SampleCountFlagBits::e1);
                    c.SetName(upsample.image, fmt::format("Reflections Upsample Image"));

                    upsample.image_view = CommonResources::CreateImageView(upsample.image, vk::ImageViewType::e2D, vk::ImageAspectFlagBits::eColor);
                    c.SetName(upsample.image_view, fmt::format("Reflections Upsample Image View"));
        ...
        void RayTracedReflections::Render(CMShared<CommandBuffer> command_buffer)
        {
            ScopedGPUProfileRaysterizer(command_buffer, "Reflections");
        ...
        void RayTracedReflections::UpdateGui()
        {
            ImGui::Checkbox("Denoise", &denoise);
            ImGui::Checkbox("Blur as Temporal Input", &temporal_accumulation.blur_as_input);
            ImGui::Checkbox("Sample GI", &ray_trace.sample_gi);
            if (ray_trace.sample_gi)
            {
                ImGui::SliderFloat("GI Intensity", &ray_trace.gi_intensity, 0.0f, 10.0f);
            }
            ImGui::Checkbox("Approximate With DDGI", &ray_trace.approximate_with_ddgi);
            if (ray_trace.approximate_with_ddgi)
            {
                ImGui::SliderFloat("Rough DDGI Intensity", &ray_trace.rough_ddgi_intensity, 0.0f, 10.0f);
            }
            ImGui::SliderFloat("IBL Indirect Specular Intensity", &ray_trace.ibl_indirect_specular_intensity, 0.0f, 1.0f);
            ImGui::InputFloat("Bias", &ray_trace.bias);
            ImGui::SliderFloat("Lobe Trim", &ray_trace.trim, 0.0f, 1.0f);
            ImGui::InputFloat("Alpha", &temporal_accumulation.alpha);
            ImGui::InputFloat("Alpha Moments", &temporal_accumulation.moments_alpha);
            ImGui::InputFloat("Phi Color", &a_trous.phi_color);
            ImGui::InputFloat("Phi Normal", &a_trous.phi_normal);
            ImGui::InputFloat("Sigma Depth", &a_trous.sigma_depth);
        }

        void RayTracedReflections::ClearImage(CMShared<CommandBuffer> command_buffer)
        {
            vk::Device device = c.GetDevice();
            RenderFrame& render_frame = c.GetRenderFrame();
            ...
            if (first_frame)
            {
                ...
                PanicIfError(command_buffer->SetImageLayout(temporal_accumulation.current_output_image[!common_resources->ping_pong], vk::ImageLayout::eUndefined, vk::ImageLayout::eGeneral, subresource_range));
                ...
                cb.clearColorImage(*temporal_accumulation.current_output_image[!common_resources->ping_pong], vk::ImageLayout::eGeneral, color, subresource_range);
                ...
                PanicIfError(command_buffer->SetImageLayout(temporal_accumulation.current_output_image[!common_resources->ping_pong], vk::ImageLayout::eGeneral, vk::ImageLayout::eShaderReadOnlyOptimal, subresource_range));
        ...
        void RayTracedReflections::Raytrace(CMShared<CommandBuffer> command_buffer)
        {
            ...
            // Ray Trace
            {
                ...
                {
                    ShaderModuleCreateInfo{ShaderModuleSourceCreateInfo::File("reflections/reflections_ray_trace.rgen")},
                    ShaderModuleCreateInfo{ShaderModuleSourceCreateInfo::File("reflections/reflections_ray_trace.rmiss")},
                    ShaderModuleCreateInfo{ShaderModuleSourceCreateInfo::File("reflections/reflections_ray_trace.rchit")},
                };
...

                {
                    ...
                    PanicIfError(ds->Bind(4, 0, common_resources->sky_environment->OutputBindings()));

                    PanicIfError(ds->Bind(5, 0, common_resources->blue_noise->GetSobolTexture()), vk::ImageLayout::eShaderReadOnlyOptimal);
                    PanicIfError(ds->Bind(5, 1, common_resources->blue_noise->GetSobolRankingTexture(BlueNoiseSpp::BLUE_NOISE_1SPP), vk::ImageLayout::eShaderReadOnlyOptimal));

                    PanicIfError(ds->Bind(6, 0, common_resources->ddgi_pass->CurrentReadBindings()));

                    PanicIfError(render_frame.FlushPendingWrites(ds));

                    ReflectionRayTracePushConstants push_constants;

                    push_constants.bias = ray_trace.bias;
                    push_constants.trim = ray_trace.trim;
                    push_constants.num_frames = current_frame;
                    push_constants.g_buffer_mip = g_buffer_mip;
                    push_constants.sample_gi = ray_trace.sample_gi && !first_frame ? 1 : 0;
                    push_constants.approximate_with_ddgi = ray_trace.approximate_with_ddgi && !first_frame ? 1 : 0;
                    push_constants.gi_intensity = ray_trace.gi_intensity;
                    push_constants.rough_ddgi_intensity = ray_trace.rough_ddgi_intensity;
                    push_constants.ibl_indirect_specular_intensity = ray_trace.ibl_indirect_specular_intensity;
                ...
                uint32_t rt_image_width = width;
                uint32_t rt_image_height = height;

                cb.traceRaysKHR(raygen_sbt, raymiss_sbt, rayhit_sbt, callable_sbt, rt_image_width, rt_image_height, 1);
            }
        ...
        void RayTracedReflections::ResetArgs(CMShared<CommandBuffer> command_buffer)
        {
...

            {
                auto shader_module_create_infos = std::vector<ShaderModuleCreateInfo>
                {
                    ShaderModuleCreateInfo{ShaderModuleSourceCreateInfo::File("reflections/reflections_denoise_reset_args.comp")},
                };
        ...
        void RayTracedReflections::TemporalAccumulation(CMShared<CommandBuffer> command_buffer)
        {
...

            {
                ...
                std::vector<vk::ImageMemoryBarrier> image_memory_barriers = {
                    vk::ImageMemoryBarrier{}
                        .setOldLayout(vk::ImageLayout::eUndefined)
                        .setNewLayout(vk::ImageLayout::eGeneral)
                        .setImage(**temporal_accumulation.current_output_image[common_resources->ping_pong])
                        .setSubresourceRange(subresource_range)
                        .setSrcAccessMask(vk::AccessFlagBits::eShaderRead)
                        .setSrcAccessMask(vk::AccessFlagBits::eShaderWrite),
                    vk::ImageMemoryBarrier{}
...

            {
                auto shader_module_create_infos = std::vector<ShaderModuleCreateInfo>
                {
                    ShaderModuleCreateInfo{ShaderModuleSourceCreateInfo::File("reflections/reflections_denoise_reprojection.comp")},
                };
                ...
                c.SetName(temporal_accumulation.pipeline_layout, "Reflections Temporal Pipeline Layout");
                c.SetName(temporal_accumulation.pipeline, "Reflections Temporal Compute Pipeline");

                cb.bindDescriptorSets(vk::PipelineBindPoint::eCompute, pl->pipeline_layout, 0, ds->descriptor_sets, {});
                cb.bindPipeline(vk::PipelineBindPoint::eCompute, cp->pipeline);

                {
                    PanicIfError(ds->Bind(0, 0, temporal_accumulation.current_output_texture[common_resources->ping_pong], vk::ImageLayout::eGeneral, vk::DescriptorType::eStorageImage));
                    ...
                    if (temporal_accumulation.blur_as_input)
                    {
                    ...
                    {
                        PanicIfError(ds->Bind(4, 0, temporal_accumulation.current_output_texture[!common_resources->ping_pong], vk::ImageLayout::eShaderReadOnlyOptimal));
                        PanicIfError(ds->Bind(4, 1, temporal_accumulation.current_moments_texture[!common_resources->ping_pong], vk::ImageLayout::eShaderReadOnlyOptimal));
                    }
                    ...
                    ReflectionTemporalAccumulationPushConstants push_constants;
        ...
        void RayTracedReflections::ATrousFilter(CMShared<CommandBuffer> command_buffer)
        {
            ...
            for (int i = 0; i < a_trous.filter_iterations; i++)
            {
                ...
                // Copy Tiles
                {
                    ScopedGPUProfileRaysterizer(command_buffer, "Copy Tiles");

                    auto shader_module_create_infos = std::vector<ShaderModuleCreateInfo>
                    {
                        ShaderModuleCreateInfo{ShaderModuleSourceCreateInfo::File("reflections/reflections_denoise_copy_tiles.comp")},
                    };
...

                    {
                        ...
                        if (i == 0)
                        {
                            PanicIfError(ds->Bind(1, 0, temporal_accumulation.current_output_texture[common_resources->ping_pong], vk::ImageLayout::eShaderReadOnlyOptimal));
                        }
                        else
                        {
                        ...
                        PanicIfError(ds->Bind(2, 2, temporal_accumulation.shadow_tile_coords_buffer));
                ...
                // A-Trous Filter
                {
                    ...
                    auto shader_module_create_infos = std::vector<ShaderModuleCreateInfo>
                    {
                        ShaderModuleCreateInfo{ShaderModuleSourceCreateInfo::File("reflections/reflections_denoise_atrous.comp")},
                    };
...

                    {
                        ...
                        ReflectionATrousFilterPushConstants push_constants;

                        push_constants.radius = a_trous.radius;
                        push_constants.step_size = 1 << i;
                        push_constants.phi_color = a_trous.phi_color;
                ...
                if (a_trous.feedback_iteration == i && temporal_accumulation.blur_as_input)
                {
        ...
        void RayTracedReflections::Upsample(CMShared<CommandBuffer> command_buffer)
        {
...

            {
                auto shader_module_create_infos = std::vector<ShaderModuleCreateInfo>
                {
                    ShaderModuleCreateInfo{ShaderModuleSourceCreateInfo::File("reflections/reflections_upsample.comp")},
                };
                ...
                c.SetName(upsample.layout, "Reflections Upsample Pipeline Layout");
                c.SetName(upsample.pipeline, "Reflections Upsample Compute Pipeline");
...

                {
                    ...
                    ReflectionUpsamplePushConstants push_constants;
        ...
        std::vector<WriteDescriptorSetBindedResource> RayTracedReflections::OutputBindings() const
        {
            if (denoise)
            {
                ...
                {
                    return { temporal_accumulation.current_output_texture[common_resources->ping_pong]};
                }
