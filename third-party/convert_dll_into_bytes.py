# edited from https://github.com/Jamesits/bin2array/blob/master/bin2array.py
import binascii
import sys
import argparse
import re
...
parser.add_argument("-N", "--filename_nsight", help="write output to a nsight file")
parser.add_argument("-A", "--filename_nsight_gpu_crash_dump", help="write output to a nsight file")
parser.add_argument("-B", "--filename_nsight_gpu_crash_dump_decoding", help="write output to a nsight file")
...
if __name__ == "__main__":
    ...
    if args.output:
        with open(args.output, 'w') as f:
            header = 'constexpr unsigned char NVIDIA_NSIGHT_DLL[] = {'
            ending = '};'
            f.write(header + ret + ending)
    else:
        print(ret)

    if args.filename_nsight:
        with open(args.filename_nsight, 'r') as f:
            file_content = f.read()
        file_content = file_content.replace('#define GFSDK_Aftermath_API extern "C" GFSDK_Aftermath_DLLSPEC GFSDK_Aftermath_Result GFSDK_AFTERMATH_CALL', '#define GFSDK_Aftermath_API extern "C" inline static GFSDK_Aftermath_Result GFSDK_AFTERMATH_CALL')
        with open(args.filename_nsight, 'w+') as f:
            f.write(file_content)

    def replace_file(filename):
        with open(filename, 'r') as f:
            file_content = f.read()
        p = re.compile(r'GFSDK_Aftermath_API (GFSDK_Aftermath_.*)\(')
        file_content = p.sub(r'GFSDK_Aftermath_API (*\1)(', file_content)
        with open(filename, 'w+') as f:
            f.write(file_content)

    if args.filename_nsight_gpu_crash_dump:
        replace_file(args.filename_nsight_gpu_crash_dump)
    if args.filename_nsight_gpu_crash_dump_decoding:
        replace_file(args.filename_nsight_gpu_crash_dump_decoding)
