...
layout(set = 6, binding = 0) uniform samplerCube s_Cubemap;
...
void main()
{
    ...
    const float roughness  = g_buffer_data_3.b;
    ...
    vec3 Wo = normalize(u_GlobalUBO.cam_pos.xyz - world_pos);

    vec4 origin = u_GlobalUBO.view_proj_inverse * vec4(0.0, 0.0, u_GlobalUBO.near_far.x, 1);
    vec4 target = u_GlobalUBO.view_proj_inverse * vec4(FS_IN_TexCoord.x, FS_IN_TexCoord.y, u_GlobalUBO.near_far.y, 1);
    origin.xyz /= origin.w;
